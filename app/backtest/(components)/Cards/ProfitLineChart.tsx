...
export default function ProfitBarChart() {
  ...
  const data = useMemo(() => {
    ...
    if (context?.record.profit && context?.dateSequence.currentDate) {
      record.current.push(context?.record.profit);
  ...
  }, [context?.dateSequence, context?.record.profit, totalDatesLen]);
